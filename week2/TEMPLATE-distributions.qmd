...
mko_clean %>% 
  # Overwrite Month_Name column - Cronilogically ordered
...
mko_clean %>% 
  ...
  #           By 'column name' what do you want to facet wrap by
  facet_wrap(~ month_name)
```
...
mko_clean |> 
  ...
  ggplot(aes(x = Temp_bot, fill = month_name)) + 
  geom_histogram(position = "identity", alpha = 0.5, color = "black", binwidth = 0.5) +
  scale_fill_manual(values = c("dodgerblue","cyan","firebrick"))
...
ggplot(mko_clean, aes(x=month_name, y = Temp_bot)) +
  geom_violin()+
  geom_boxplot(width = 0.1, fill = "gray", color = "grey30", alpha = 0.5)+
  scale_x_discrete(limit = rev(month.name))+
  coord_flip()
```
