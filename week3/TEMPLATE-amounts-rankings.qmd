...
order_2015 <- heatmap_data %>% 
  ...
  mutate(order_on_the_court = row_number()) %>% 
  select(subregion_name, order_on_the_court)

#........join order with rest of data to set factor levels.......

heatmap_order <- heatmap_data %>% 
  left_join(order_2015) %>% 
  mutate(subregion_name = fct_reorder(.f= subregion_name, .x=order_on_the_court))
...
ggplot(heatmap_order, aes(x = year, y= subregion_name, fill = annual_mean_sui)) +
  ...
  labs(fill = "Annual Mean Surface Water-Supply \nand Use Index (SUI)")+
